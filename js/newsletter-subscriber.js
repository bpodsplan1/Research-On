...
// 정기 발송 스케줄: 매주 화요일 08시 후보 수집 → 관리자 검토 → 09시 발송(2026-08-03 확정)
function computeNextNewsletterSchedule(){
  const now = new Date();
  const daysUntilTuesday = (2 - now.getDay() + 7) % 7 || 7;
  const next = new Date(now); next.setDate(now.getDate() + daysUntilTuesday);
  return next.toLocaleDateString('ko-KR') + ' <span style="white-space:nowrap">(화요일 발송 예정)</span>';
}
